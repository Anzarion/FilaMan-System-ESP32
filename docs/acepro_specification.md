...
1. ~~Only SKU and Color matter for the printer~~ **CORRECTED (tested 2026-06-07):** ACE Pro V1.3.863 reads and uses SKU, type, color, extruder/bed temps, brand and diameter (all visible via get_filament_info)
...
**CORRECTED (2026-06-07):** The ACE Pro reads this field and reports it as extruder_temp {min, max}.
...
**CORRECTED (2026-06-07):** The ACE Pro reads this field and reports it as hotbed_temp {min, max}.
...
**EMPIRICALLY CONFIRMED (2026-06-07):** The length field is ignored by ACE Pro V1.3.863.
Test: length changed to 165 in isolation (all other bytes standard, dump-verified) ->
get_filament_info still reports total=330. total/current are firmware constants
(330/0), independent of tag contents.

### Page 31: Weight (4 bytes)
```
Byte [0-1]: Weight (U16 Little-Endian, in grams) - presumed; DnG-Crafts lists
            the whole page as "unknown"
...
**EMPIRICALLY CONFIRMED (2026-06-07):**
- Bytes [0-1] (weight): ignored by the ACE (tags with 500g/250g -> display unchanged at 0/330m)
- Bytes [2-3]: neither validated nor read by the ACE (test: wrote 0x01F4=500 ->
  tag accepted as identified, current stays 0). Still write 0x0000 (compatibility).
...
## EMPIRICAL FINDINGS: ACE Pro behavior (as of 2026-06-07, FW V1.3.863)

Results of a systematic test series (isolated single changes, dump-verified,
FilaMan scale as the writer, ACE Pro attached via the Klipper/ACEPRO integration):

### total / current in get_filament_info

| Field | Source | Controllable via tag? |
|------|--------|------------------------|
| `total` | firmware constant 330 (m) | **No** - neither the length field (page 30, tested with 165) nor the weight field (page 31, tested with 500g/250g) change the reported value |
| `current` | ACE-internal consumption counter | **No** - page 31 bytes [2-3] are ignored (tested with 500); no tag field exists |

**Consequence:** Real fill levels in the ACE panel are only possible via external
synchronization (e.g. a FilaMan live sync), not via tag contents.

### RFID state cache (important for debugging!)

The ACE caches the RFID read state **per tag UID** persistently:
- A tag once classified as faulty stays `rfid=failed`, even after the tag
  contents have been repaired
- **Reset only via power-cycling the ACE** (pull the plug)
- Additionally, the ACEPRO Klipper integration persists slot inventory
  (`ACE_RESET_PERSISTENT_INVENTORY` to reset); its rfid flag used to latch
  on failures (optimistic set before the callback response)

### Strict requirements on tag contents

- All unwritten bytes inside the fields MUST be 0x00. Garbage bytes (e.g. from
  uninitialized buffers) after a string's null terminator cause `rfid=failed`
  (the whole tag is rejected)
- Recommendation: zero the entire user area before every write (clearNtagUserData)

### Format insights from community sources (2026)

- **Page 4, byte 2 = format version:** 0x64 = v1 (no brand field), 0x65 = v2
  (with brand, pages 10-14) - source: Molodos
- **Pages 21-22:** candidates for color 2/3 (multicolor), unconfirmed - source: Molodos
- **Pages 23-28:** used on high-speed PLA tags (print speed ranges 1-3 plus
  nozzle temp ranges 2/3) - 0x00 on standard tags - source: Molodos
- **SKU naming scheme:** A=Anycubic series, H=header, PL/PLP/HS/PE/AB/TP=material,
  2-3 letter color code, -10x revision (e.g. AHHSBK-103 = Anycubic PLA High Speed
  Black) - source: mrRobot62/anycubic_filament_sku_sniffer
- The Anycubic slicer matches via SKU (workbench) and the type string (material presets)

### Sources

- https://github.com/DnG-Crafts/ACE-RFID (tag layout, N033 protocol, 20+ original dumps)
- https://github.com/Molodos/anycubic-nfc-filament (format.md: versions, HS layout)
- https://github.com/mrRobot62/anycubic_filament_sku_sniffer (SKU_DEMYSTIFY.md)
- Own test series 2026-06-06/07 (this repo, branch port-acepro)
