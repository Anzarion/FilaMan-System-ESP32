...
// ===== ACE Pro Hybrid Mode (persistent toggle) =====
#include <Preferences.h>
bool aceProHybridMode = false;

void loadAceProMode() {
    Preferences prefs;
    prefs.begin("acepro", true);
    aceProHybridMode = prefs.getBool("hybrid", false);
    prefs.end();
    Serial.printf("[ACEPro] Hybrid mode loaded: %s\n", aceProHybridMode ? "ON" : "OFF");
}

void setAceProMode(bool enabled) {
    aceProHybridMode = enabled;
    Preferences prefs;
    prefs.begin("acepro", false);
    prefs.putBool("hybrid", enabled);
    prefs.end();
    Serial.printf("[ACEPro] Hybrid mode set: %s\n", enabled ? "ON" : "OFF");
}
...
String getSKU(const JsonObject& spoolData) {
    String vendor = String(spoolData["brand"] | spoolData["filament"]["vendor"]["name"] | "GENERIC");
    String material = String(spoolData["type"] | spoolData["filament"]["material"] | "PLA");
    String spoolId = String(spoolData["spool_id"] | spoolData["id"] | 0);
...
String getBrand(const JsonObject& spoolData) {
    String brand = String(spoolData["brand"] | spoolData["filament"]["vendor"]["name"] | "Generic");
...
uint32_t getColor(const JsonObject& spoolData) {
    ...
    else if (spoolData["color_hex"].is<const char*>() || spoolData["filament"]["color_hex"].is<const char*>()) {
        const char* colorHexPtr = spoolData["color_hex"].is<const char*>() ? (const char*)spoolData["color_hex"] : (const char*)spoolData["filament"]["color_hex"];
...
bool extractACEProData(const JsonObject& spoolData, ACEProData& aceData) {
    memset(&aceData, 0, sizeof(ACEProData));  // FIX: prevent uninitialized stack data in string fields (page 19 garbage)
    ...
    String colorHex = String(spoolData["color_hex"] | spoolData["filament"]["color_hex"] | "#FFFFFF");
    ...
    aceData.nozzleTempMin = (uint16_t)(String(spoolData["min_temp"] | "").toInt() ?: (int)(spoolData["filament"]["nozzle_temp_min"] | 200));
    aceData.nozzleTempMax = (uint16_t)(String(spoolData["max_temp"] | "").toInt() ?: (int)(spoolData["filament"]["nozzle_temp_max"] | 230));
    ...
    String spoolIdStr = String(spoolData["spool_id"] | spoolData["id"] | 0);
 ...
 */
void startWriteNfcTagBinary(const char* spoolId, const char* payloadJson) {
    ...
    // Allocate parameter struct (will be freed in task)
    AceProWriteParams* params = new AceProWriteParams();
    params->spoolId = spoolIdInt;
    params->payload = String(payloadJson ? payloadJson : "");
...
void writeNfcTagBinaryTask(void* param) {
    AceProWriteParams* taskParams = (AceProWriteParams*)param;
    int spoolId = taskParams->spoolId;
    String payloadStr = taskParams->payload;
    delete taskParams;  // Free parameter memory
    ...
    // Step 1: Parse spool data from payload (provided by FilaMan backend)
    Serial.println("[ACEPro] Step 1: Parsing spool data from payload...");
    JsonDocument spoolData;
    DeserializationError parseError = deserializeJson(spoolData, payloadStr);
    
    if (parseError || spoolData.isNull()) {
        Serial.println("[ACEPro] Failed to parse spool data payload");
        sendRfidResultAsync("", spoolId, 0, false, "Invalid ACE Pro payload");
    ...
    bool tagFound = false;
    for (int attempt = 0; attempt < 16 && !tagFound; attempt++) {   // ~5s Gesamtfenster
        tagFound = nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength, 300);
        if (!tagFound) vTaskDelay(pdMS_TO_TICKS(100));
    }
    if (!tagFound) {
        Serial.println("[ACEPro] No tag detected after retries");
        sendRfidResultAsync("", spoolId, 0, false, "Timeout - no tag found");
        nfcReaderState = NFC_WRITE_ERROR;
        nfcWriteInProgress = false;
        vTaskDelete(NULL);
        return;
    ...
    for (uint8_t i = 0; i < uidLength; i++) {
        if (uid[i] < 0x10) uidString += "0";   // zero padding, matches v3 scan path
        uidString += String(uid[i], HEX);       // lowercase - backend normalizes to lower
        if (i < uidLength - 1) {
            uidString += ":";
        }
    }
    ...
    Serial.println("[ACEPro] Step 4b: Clearing tag user data (full wipe before write)...");
    if (!clearNtagUserData()) { Serial.println("[ACEPro] WARNING: Tag clearing incomplete - continuing"); }
    ...
    if (writeSuccess) {
        ...
        // Report tag UID to FilaMan backend (updates spools.rfid_uid)
        sendRfidResultAsync(uidString, spoolId, 0, true, "");
    } else {
        Serial.println("[ACEPro] ✗ Tag write FAILED");
        nfcReaderState = NFC_WRITE_ERROR;
        sendRfidResultAsync("", spoolId, 0, false, "ACE Pro binary write failed");
    }
